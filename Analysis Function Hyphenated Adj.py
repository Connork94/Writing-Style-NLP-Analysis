...
#from NLPfunctions import synonym_analysis
...
target_words_adj = ["awful", "pale", "supple", "dynamic", "terrific", "mighty", "foul", "red", "white", "black",
                    ...
                    "primitive", "yore", "crazy", "dark", "dead", "wild", "strange", "mad", "grim", "terrible",
                    "white-faced", "frightful", "slender", "powerful", "powerful", "muscular", "brawny", "wan", "wondrous"]

target_words_verb = ["fling","gaze","ask","hack","shrug","fade","answer","count","clinch","slay",
                     "shrink","slug","ruminate","writhe","haunt",
                     "demand","pull","pant","assure","grab","glimpse","shriek","stumble","howl","jump","leap",
                     "bound","spring", "toss", "strike", "smash", "hit", "cut", "rip", "rend", "murder", "puff",
                     "tear", "ask", "gasp", "heave"]
...
def synonym_analysis(df, folder_dataset, word_type):
    ...
    for word in synonyms_dict:
        ...
        if count in [100, 500, 1000, 2000, 3000, 4000, 5000, 6000, 8000, 10000]:
            print(count)
        else:
            pass
    try:
        synonyms_scores_dict_scores_only = {}
        print("creating synonym dict scores")
        for word in synonyms_dict:
            # print(word)
            count += 1
    ...
    print("creating lists for final df")
    ...
    final_df = pd.DataFrame.from_dict(df_prep)
    ...
    file_name = folder_dataset + word_type + 's' + '_' + 'Words_and_differences_' + datetime.now().strftime(
        "%Y%m%d-%H%M%S") + '.xlsx'
    final_df.to_excel(file_name, sheet_name='sheet1', index=False)
    print(file_name)
...
def analysis(word_type, list_of_codes, stop_words, target_words, folder_texts, folder_images):
    ...
    # save to csv file
    file_name_csv = folder_dataset + word_type + 's' + '_' + 'weightedscore_freq_' + datetime.now().strftime(
        "%Y%m%d-%H%M%S") + '.csv'
    data1.to_csv(file_name_csv)

    # save to PKL
    file_name_pkl = folder_dataset + word_type + 's' + '_' + 'weightedscore_freq' + '.pkl'
    data1.to_pickle(file_name_pkl)

    # synonym analysis
    if synonyms == 1:
        synonym_analysis(data1, folder_dataset, word_type)
    else:
        pass
